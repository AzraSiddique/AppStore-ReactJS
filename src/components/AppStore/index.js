...
class AppStore extends Component {
  ...
  filteredApps = () => {
    ...
    const filteredAppsByTab = appsList.filter(
      eachApp => eachApp.category === activeTabId,
    )
    const filteredResults = filteredAppsByTab.filter(AppDetails =>
      AppDetails.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
  ...
  render() {
    const {activeTabId, searchInput} = this.state
    const filteredResults = this.filteredApps()
